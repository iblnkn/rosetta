...
def _decode_image_by_encoding(
    ...
    raise ValueError(f"Unsupported image encoding: '{enc}'. Supported: {sorted(SUPPORTED_IMAGE_ENCODINGS)}")
...
def decode_ros_image(msg) -> np.ndarray:
    ...
    return _decode_image_by_encoding(enc, raw, h, w, step)
...
def _dec_twist_stamped(msg: Any, spec: ObservationStreamSpec) -> np.ndarray:
    ...
    if not spec.names:
        return np.concatenate(
            [
                np.array([msg.twist.linear.x, msg.twist.linear.y, msg.twist.linear.z], dtype=np.float64),
                np.array(
...
def _dec_joint_trajectory(msg: Any, spec: ObservationStreamSpec) -> np.ndarray:
    ...
    for selector in spec.names:
        ...
        if attr is None:
            raise ValueError(
                f"Unknown JointTrajectoryPoint field '{field}'. Valid fields: position, velocity, acceleration, effort"
            )
        if joint_name not in joint_to_idx:
            raise ValueError(f"Joint '{joint_name}' not in message. Available: {list(msg.joint_names)}")
...
def _dec_joy(msg: Any, spec: ObservationStreamSpec) -> np.ndarray:
    ...
    for selector in spec.names:
        ...
        if field == "axes":
            if idx >= len(msg.axes):
                raise ValueError(f"Axis index {idx} out of range (len={len(msg.axes)})")
            out.append(float(msg.axes[idx]))
        elif field == "buttons":
            if idx >= len(msg.buttons):
                raise ValueError(f"Button index {idx} out of range (len={len(msg.buttons)})")
