...
from rosetta.ros2.ros2_utils import qos_profile_from_dict, wait_until
...
class RosettaHilManagerNode(LifecycleNode):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            ...
            '/record_episode',
            ParameterDescriptor(
                description='Action name for episode recorder client', read_only=True
            ),
        )
        self.declare_parameter(
            'policy_remap_prefix',
        ...
        if enable_reward and self._contract.rewards:
            for reward_spec in self._contract.rewards:
                ...
                sub = self.create_subscription(
                    msg_cls,
                    remapped_topic,
                    lambda msg, topic=original_topic: self._on_reward_classifier_output(
                        msg, topic
                    ),
                    qos,
                    callback_group=self._sub_cbg,
