...
the frame dicts to a :class:`~rosetta.backends.protocols.DatasetWriter` chosen
by ``--backend``.

Usage:
    # LeRobot dataset (default backend)
    python -m rosetta.ros2.port \
        --raw-dir ./datasets/bags --contract contract.yaml \
        --repo-id my_org/my_dataset --root ./datasets/lerobot

    # vla_foundry tar shards
    python -m rosetta.ros2.port --backend vla_foundry \
        --raw-dir ./datasets/bags --contract contract.yaml \
...
from rosetta.backends.protocols import available_backends, DATASET_WRITER_GROUP, load_dataset_writer
from rosetta.core.contract import load_contract
from rosetta.core.contract_utils import iter_specs
from rosetta.ros2.bag_frames import BagFrameSource, iter_bag_frames
...
def port(
    ...
    *,
    ...
    """Port rosbag2 recordings to a backend dataset."""
    contract = load_contract(contract_path)
    specs = list(iter_specs(contract))

    source = BagFrameSource(raw_dir, specs, num_shards=num_shards, shard_index=shard_index)
    bag_dirs = source.bag_dirs()
    if not bag_dirs:
        logging.warning('No bags to process in this shard')
        return

    writer = load_dataset_writer(backend)
    writer.open(
        contract=contract,
        specs=specs,
        repo_id=repo_id,
    ...
    for episode_index, bag_dir in enumerate(bag_dirs):
        ...
        try:
            frame_count = 0
            for frame in iter_bag_frames(bag_dir, specs):
...
def main():
    ...
    parser.add_argument(
        '--backend',
        default='lerobot',
        help='Dataset backend (default: lerobot). Installed: '
        + (', '.join(available_backends(DATASET_WRITER_GROUP)) or '(none)'),
    )
    parser.add_argument('--raw-dir', type=Path, required=True, help='Directory of bag subdirs')
    parser.add_argument(
        '--repo-id', type=str, default=None, help='Dataset repo ID (default: raw-dir name)'
    )
    ...
    # Common and backend-specific writer options. Writers ignore options they
    # don't use.
    ...
    parser.add_argument(
        '--image-indices', type=str, default=None, help='[vla_foundry] comma list, e.g. "-1,0"'
    )
    parser.add_argument(
        '--samples-per-shard', type=int, default=None, help='[vla_foundry] samples per tar shard'
    )
